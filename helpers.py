...
def mixMaterials(material1, material2, alpha):
    # material1: (1,256,256,3)
    normal1Corrected = (material1[:, 0] - 0.5) * 2.0 #go between -1 and 1 (1, 256, 256, 3)
    print("normal1Corrected-mixMaterials: {}".format(normal1Corrected.eval().shape))
    normal2Corrected = (material2[:, 0] - 0.5) * 2.0
    mag1 = tf.expand_dims(tf.maximum(0.01, normal1Corrected[:,:,:,2]), axis = -1)# (1, 256, 256, 1)
    mag2 = tf.expand_dims(tf.maximum(0.01, normal2Corrected[:,:,:,2]), axis = -1)# (1, 256, 256, 1)
    print("mag1-mixMaterials: {}".format(mag1.eval().shape))
    print("mag2-mixMaterials: {}".format(mag2.eval().shape))
    normal1Projected = normal1Corrected / mag1 #Project the normals to use the X and Y derivative
    normal2Projected = normal2Corrected / mag2
    print("normal1Projected: {}".format(normal1Projected.eval().shape))
    mixedNormals = alpha * normal1Projected  + (1.0 - alpha) * normal2Projected# (1, 256, 256, 3)
    print("mixedNormals: {}".format(mixedNormals.eval().shape))
    normalizedNormals = mixedNormals / tf.sqrt(tf.reduce_sum(tf.square(mixedNormals), axis=-1, keep_dims=True))
    print("normalizedNormals: {}".format(normalizedNormals.eval().shape))
    normals = (normalizedNormals * 0.5) + 0.5 # Back to 0;1
    print("normals: {}".format(normals.eval().shape))#  (1, 256, 256, 3)
    mixedRest = alpha * material1[:, 1:] + (1.0 - alpha) * material2[:, 1:]# (1, 3, 256, 256, 3)
    print("mixedRest: {}".format(mixedRest.eval().shape))
    final = tf.concat([tf.expand_dims(normals, axis = 1), mixedRest], axis = 1)# (1, 4, 256, 256, 3)
    print("final: {}".format(final.eval().shape))
    return final
