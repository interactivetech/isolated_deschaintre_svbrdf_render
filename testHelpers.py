...
def renderTests(input_dir, testFolder, maxInputNb, tmpFolder, imageFormat, CROP_SIZE, nbTargets, input_size, batchSize, renderingScene, jitterLightPos, jitterViewPos, inputMode, mode, outputDir):
    ...
    data = dataReader.dataset(input_dir, imageFormat = imageFormat, testFolder = testFolder, maxInputToRead = maxInputNb, nbTargetsToRead = nbTargets, cropSize=CROP_SIZE, inputImageSize=input_size, batchSize=batchSize, fixCrop = True, mixMaterials = True, fixImageNb = True, logInput = False, useAmbientLight = False, jitterRenderings = False, useAugmentationInRenderings = False)
